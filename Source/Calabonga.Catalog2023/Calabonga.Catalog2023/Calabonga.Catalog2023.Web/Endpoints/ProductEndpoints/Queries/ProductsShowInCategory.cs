...
public class ProductsShowInCategory
{
	...
	public class Handler : IRequestHandler<Request, Unit>
	{
		...
		public async Task<Unit> Handle(Request request, CancellationToken cancellationToken)
		{
			...
			var products = productRepository
				.GetAll(
					predicate: x => x.CategoryId == request.CategoryId && x.Visible == false,
			...
			products.ForEach(x => x.Visible = true);
			productRepository.Update(products);
			await _unitOfWork.SaveChangesAsync();

			foreach (var product in products)
			{
				await _mediator.Publish(new ProductShow.Notification(product.Id, request.User), cancellationToken);
			}
